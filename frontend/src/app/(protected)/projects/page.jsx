    ...
    <div className="flex-1 p-4 max-w-6xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {projects.map((project) => (
          ...
          >
            ...
            <p className="text-sm text-slate-600 mb-4 h-10 line-clamp-2 dark:text-slate-400">{project.description}</p>

            <div className="text-xs text-slate-500 dark:text-slate-500">
              Lead: <span className="font-medium text-slate-700 dark:text-slate-300">{project.lead}</span>
            </div>
          </Link>
        ))}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50">
          <div className="bg-white rounded-md shadow-lg w-full max-w-md p-3 dark:border dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-md transition-colors font-medium dark:text-slate-300 dark:hover:bg-slate-800"
              >
                Cancel
              </button>
              <button
